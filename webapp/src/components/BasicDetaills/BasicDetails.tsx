...
const BasicDetails = () => {
 ...
 admissionIdProofFiles: { file?: File; url?: string; name?: string }[];
  }>({
    showModal: false,
    croppedImage: "",
    loading: false,
    ...
    // admission ID proof state
  admissionIdProofFiles: []
  });
...
const handleChange = useCallback((e: React.SyntheticEvent) => {
  ...
  if (type === "file" && files?.length) {
    if (name === "patientPic") {
      // Single profile photo logic
      const maxSize = 2 * 1024 * 1024; // 2 MB
      if (files[0].size > maxSize) {
        if (fileInputRef.current) fileInputRef.current.value = "";
        handleError(new Error("Image size exceeds 2 MB limit."));
        return;
      }
    ...
    else if (name === "admissionDocuments") {
      // Multiple documents logic
      const maxSize = 5 * 1024 * 1024; // 5 MB per file
      const validFiles = Array.from(files).filter((file) => {
        if (file.size > maxSize) {
          handleError(new Error(`${file.name} exceeds 5 MB limit.`));
          return false;
        }
        return true;
      });
      ...
      setState((prev) => {
        const existingNames = new Set(prev.admissionIdProofFiles.map((f) => f.name));
        ...
        return {
          ...prev,
          admissionIdProofFiles: [...prev.admissionIdProofFiles, ...newFiles],
        };
      });
    }
  } 
    ...
    if (isNumeric(value)) {
      setState((prev) => ({ ...prev, age: +value, dob: "" }));
    }
  } 
  ...
  const updatePateintData = (pid: string) => {
    ...
    if (typeof state.patientPic == "string" && state.patientPic.trim() === "")
      formData.append("patientPic", state.patientPic);
  if (state.admissionIdProofFiles && state.admissionIdProofFiles.length > 0) {
    state.admissionIdProofFiles.forEach((f) => {
      if (f.file) {
        formData.append("admissionIdProofFiles", f.file);
      }
    });
  }
    ...
    console.log("Updating patient with data:", ...formData.entries());

    return updatePatient(formData, pid);
  };
const updatePateintAddmissionData = async (pid: string, aid: string) => {
  const updatedPatientData = compareObjects(patientData.patientAdmission, state, true);
  const payload: { [key: string]: unknown } = {};

  // involuntary admission type
  if (
  ...
  // agar na payload change hua aur na files hain, return
  if (!Object.entries(payload).length && (!state.admissionIdProofFiles || state.admissionIdProofFiles.length === 0)) {
    return;
  }

  // agar multiple files hain to FormData banao
  if (state.admissionIdProofFiles && state.admissionIdProofFiles.length > 0) {
    const formData = new FormData();
    ...
    return updateSinglePatinetAdmissionHistory(formData as unknown as any, pid, aid);
  }

  // agar koi file nahi hai to normal payload bhejo
  return updateSinglePatinetAdmissionHistory(payload, pid, aid);
};


  // ---------- New handlers for admission ID proof upload ----------
const handleDropAdmissionFiles = useCallback((files: File[]) => {
  ...
  try {
    ...
    setState((prev) => {
      ...
      const updatedFiles = [...prev.admissionIdProofFiles, ...newFiles];

      // ✅ Redux me bhi bind kar do
      dispatch(
        setPatientDetails({
          ...patientData.patientDetails,
          admissionIdProofFiles: updatedFiles
        })
      );
...
}, [dispatch, stepperData.discardModal.isFormChanged, patientData.patientDetails]);


const handleDeleteAdmissionFile = useCallback((index: number) => {
  setState((prev) => {
    const updatedFiles = [...prev.admissionIdProofFiles];
    updatedFiles.splice(index, 1); // index से remove
    return { ...prev, admissionIdProofFiles: updatedFiles };
  });
  ...
  try {
    ...
    if (patientData.patientDetails._id) {
      ...
      if (patientResponse?.data?.status === "success") {
        ...
        // Fetch admission proof files from backend if available
        const admissionFiles =
          patientResponse?.data?.data?.admissionIdProofFiles?.map((f: any) => ({
            url: f.url,
            name: f.name,
            _id: f._id
          })) || state.admissionIdProofFiles;

        setState((prev) => ({
          ...prev,
          patientPic: patientResponse?.data?.data?.patientPicUrl || patientResponse?.data?.data?.patientPic,
          admissionIdProofFiles: admissionFiles
        }));

        // Update patient details in redux
        dispatch(
          setPatientDetails({
            ...patientData.patientDetails,
            ...
            referredTypeId,
            referralDetails,
            admissionIdProofFiles: admissionFiles // ✅ Bind files here
          })
        );
      }
      ...
      if (patientData.patientAdmission._id && patientData.patientAdmission.patientId) {
        const admissionResponse = await updatePateintAddmissionData(
          patientData.patientAdmission.patientId,
          patientData.patientAdmission._id
        );

        if (admissionResponse?.data?.status === "success") {
          setState((prev) => ({
            ...prev,
            admissionIdProofFiles:
              admissionResponse?.data?.data?.admissionIdProofFiles?.map((f: any) => ({
                url: f.url,
                name: f.name,
                _id: f._id
              })) || prev.admissionIdProofFiles
          }));

          dispatch(
            setPatientAdmission({
              ...patientData.patientAdmission,
              _id: admissionResponse?.data?.data?._id,
              dateOfAdmission: state.dateOfAdmission,
              time: state.time,
              admissionType: state.admissionType,
              involuntaryAdmissionType: state.involuntaryAdmissionType
            })
          );
        }
      } else {
        const admissionResponse = await createPateintAddmissionData(
          patientData.patientDetails._id,
          state
        );
        if (admissionResponse?.data?.status === "success") {
          const resourceData = await addDefaultResource(
            patientData.patientDetails._id,
            admissionResponse?.data?.data?._id
          );
      ...
      if (patientResponse.data.status === "success") {
        ...
        setState((prev) => ({
          ...prev,
          patientPic: patientResponse?.data?.data?.patientPicUrl || state.patientPic
        }));
        ...
        dispatch(
          setPatientDetails({
            ...
            referredTypeId,
            referralDetails,
            admissionIdProofFiles: state.admissionIdProofFiles // ✅ new patient files
          })
        );
      }
    }
    ...
    if (err instanceof Error && "inner" in err) {
      ...
      validationErrorArray.forEach((e) => {
        if (e.path && !validationErrors[e.path]) validationErrors[e.path] = e.message;
      });
  ...
  try {
    ...
    if (patientData.patientDetails._id) {
      ...
      if (patientResponse?.data?.status === "success") {
        ...
        // Backend se admission files le lo agar available ho
        const admissionFiles =
          patientResponse?.data?.data?.admissionIdProofFiles?.map((f: any) => ({
            url: f.url,
            name: f.name,
        ...
        // Redux me patient details update karo
        dispatch(
          setPatientDetails({
            ...patientData.patientDetails,
            uhid: patientResponse?.data?.data?.uhid,
            ...
            referredTypeId,
            referralDetails,
            admissionIdProofFiles: admissionFiles // ✅ Files bind ho rahi hain
          })
        );
      }
      ...
      if (patientResponse.data.status === "success") {
        ...
        dispatch(
          setPatientDetails({
            ...
            referredTypeId,
            referralDetails,
            admissionIdProofFiles: state.admissionIdProofFiles // ✅ Bind files for new patient
          })
        );
      }
    }
        ...
        <div className="grid lg:grid-cols-2 md:grid-cols-1 w-full items-start gap-20">
          ...
          <div className="w-full">
            ...
            <div className="flex w-fit gap-4 flex-wrap">
              ...
              {/* ------------ ID Proof upload (copied from ProfileContacts) ------------ */}
<CheckBox
  name=""
  checked={true}

  filesString={
    state.admissionIdProofFiles.map((f) =>
      f.url
        ? {
            filePath: f.url,
            fileUrl: f.url,
            fileName: f.name || "Admission Proof"
          }
        : undefined
  ...
  ContainerClass="-ml-5"
  checkHide
  handleDeletes={(index) => handleDeleteAdmissionFile(index)}
  handleDrop={(files) => handleDropAdmissionFiles(files)}
  handleCheck={() => {}}
/>
