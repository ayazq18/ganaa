...
const GroupActivity = () => {
  ...
  const fetchAllPatient = async () => {
    ...
    try {
      ...
      console.log("hii")
      console.log( "tabsdatareponse",tabsdatareponse);
      ...
      //   ])
      // );
      const selectedCenterId = selected === "All" ? null : selected;

const updateMap = new Map(
  tabsdatareponse?.data?.data[0]?.tabInfo
    ?.filter((item: { name: string; note: string }) => {
      // agar "All" select hai -> sab notes dikhana
      if (selectedCenterId === null) return true;

      // warna bas wahi notes dikhna jinka center match kare
      return tabsdatareponse?.data?.data[0]?.selectedCenter?.id === selectedCenterId;
    })
    .map((item: { name: string; note: string }) => [item.name, item.note])
);
 ...
 const handleCheckboxChange = async (
  ...
  if (!activityItem.isSelected) {
    // Unchecked: clear note + submit
    activityItem.note = "";
    await handleSubmit(patient?.patientId, "remove"); 
    setHoveredIndex(null);
  } else {
    // Checked: submit immediately
    await handleSubmit(patient?.patientId, "add"); 
  }
};
  ...
  const handleSubmit = async (id: string | undefined, action: "add" | "remove" = "add") => {
    ...
    if (filterData[0]._id) {
      const payload = filterData.map((value) => ({
        ...
        activity: value?.activity
          ?.filter((act) => act.isSelected || act.note?.trim())
          .map((data) => ({
            name: data.name,
            note: data.note,
      ...
      if (response.data.status === "success") {
        ...
        toast.success(
          action === "remove"
            ? "Group activity removed successfully"
            : "Group activity added successfully"
        );
      }
    } else {
      const payload = filterData?.map((value) => ({
  ...
  //     console.log("hii the data is here ")
  ...
  try {
    ...
    if (tabdata._id) {
      const response = await updateNewGroupActivitytabs(tabdata._id, payload);
      ...
      const response = await createNewGroupActivitytabs(payload);
    ...
    prevData.map((row) => {
      ...
      return {
        ...
        activity: row.activity?.map((activityItem) => {
          ...
          // Keep the existing isSelected if user toggled checkbox,
          // but also mark selected if user added a note.
          const resultingIsSelected = Boolean(activityItem.isSelected) || (trimmedNote?.length > 0);

          return {
            ...activityItem,
            note: remeberstrings,
            isSelected: resultingIsSelected
          };
        })
      };
    })
                                    ...
                                    <div className="relative  w-fit gap-1 mx-auto flex items-center justify-center">
                                      ...
                                      <div>
                                        <div className="relative w-full">
                                          <img
                                            onClick={() => {
                                              // always open the popup — if there's an existing note, open view/edit mode,
                                              // otherwise open the editable textarea (state === "").
                                              setRememberstring(cell?.note || "");
                                              setHoveredIndex({
