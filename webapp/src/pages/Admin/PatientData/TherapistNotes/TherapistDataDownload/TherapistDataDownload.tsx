...
import { ITherapistState } from "./types";
import { getAllTherapistNotes } from "@/apis";
...
const TherapistDataDownload = ({
  patientDetails,
  data,
  button,
  aid
}: {
  patientDetails: ITherapistState;
  data?: any;
  aid?: string;
  ...
  const generatePdf = async (data: any) => {
    ...
    try {
      ...
      if (!data && aid) {
        ...
        const { data: therapistData } = await getAllTherapistNotes({
          limit: 500,
          page: 1,
          sort: "-createdAt",
...

            {
              table: {
                ...
                body: [
                  ...
                  ...(Array.isArray(therapistData?.data) ? therapistData.data : []).map(
                    (d: any) => {
                      return [
                        {
        ...
        pdfMake
          ...
          .download(`therapist-notes-${formatId(patientDetails.UHID, false)}-${formattedDate}.pdf`);
        setloading(false);
      } else {
        setloading(true);
        ...
        pdfMake
          .createPdf(docDefinition)
          .download(`therapist-note-${formatId(patientDetails.UHID, false)}-${formattedDate}.pdf`);
...
export default TherapistDataDownload;
