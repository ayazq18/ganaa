...
const upload = multer({
  ...
  fileFilter: MFileFilter.patientPicAndAdmissionFilesFilter,
});

export const uploadPatientFiles = upload.fields([
  { name: "patientPic", maxCount: 1 },              // Single profile pic
  { name: "admissionIdProofFiles", maxCount: 10 },  // Multiple files
]);
...
export const updateSinglePatient = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    console.log("🟢 Incoming request for update patient:", req.params.id);
    console.log("🟢 Body fields:", Object.keys(req.body));
    console.log("🟢 Files received:", req.files ? Object.keys(req.files as any) : "No files");

    // Normal field validations
    if (Helper.isExist(req, "firstName") && req.body.firstName.length === 0)
      return next(new AppError("First Name is Mandatory", 400));
    ...
    if (req.body.firstName)
      req.body.firstName = Helper.normalizeAndTitleCase(req.body.firstName);
    if (req.body.lastName)
      req.body.lastName = Helper.normalizeAndTitleCase(req.body.lastName);
    ...
    // Handle Profile Pic
    if (req.files && (req.files as any).patientPic) {
      const file = (req.files as any).patientPic[0];
      console.log("📷 Patient Pic received:", file.originalname, file.mimetype, file.size);
      ...
      console.log("⚠️ No patientPic found in request");
    }

    // 🔹 Fetch existing patient doc first
    const patientPreviousDoc = await Patient.findById(req.params.id).lean();
    console.log("🔍 Fetched previous patient document for revision check:", patientPreviousDoc);
    if (!patientPreviousDoc)
      return next(new AppError("Please Provide Valid Patient ID", 400));

    // Handle Admission ID Proof Files - PUSH instead of REPLACE
    if (req.files && (req.files as any).admissionIdProofFiles) {
      const docs = (req.files as any).admissionIdProofFiles;
      console.log("📑 AdmissionIdProofFiles received:", docs.length);

      // Start with existing files
      filterObj.admissionIdProofFiles = patientPreviousDoc.admissionIdProofFiles || [];

      for (const doc of docs) {
        if (filterObj.admissionIdProofFiles.length >= 10) {
          console.log("⚠️ Maximum 10 Admission ID Proof files reached. Skipping extra files.");
          break;
        }

        const fileName = `${Date.now()}-${random.randomAlphaNumeric(6)}-${doc.originalname}`;
        const filePath = S3Path.patientPic(req.params.id ?? "", fileName);

        await S3.uploadFile(filePath, doc.buffer, doc.mimetype);

        filterObj.admissionIdProofFiles.push({
          fileName,
          fileUrl: filePath,
        });

        console.log("✅ Uploaded admissionIdProofFile:", filePath);
      }
    } else {
      console.log("⚠️ No admissionIdProofFiles found in request");
    }
    ...
    console.log("✅ Patient updated successfully in DB:", data._id);
    console.log("data is:", data);

    // 🔹 Safe admission history mapping
    const admissionHistoryMap = await PatientAdmissionHistory.getLatestPatientHistory([data._id as ObjectId]);
    const patientIdStr = data._id?.toString() || "";
    const patientHistory = admissionHistoryMap[patientIdStr] || {};
    const resourceAllocation = patientHistory?.resourceAllocation || [];
