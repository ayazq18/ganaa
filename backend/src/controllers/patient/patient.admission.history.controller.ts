...
export const uploadPatientAdmissionChecklist = uploadMemoryFields.fields([
  { name: 'applicationForAdmission', maxCount: 5 },  { name: 'admissiondocuments', maxCount: 5 },
...
export const updateSinglePatientAdmissionHistory = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    // Only remove fields you really don't want updated
    ...
    if (!admissionHistoryDoc.isSuccess || !admissionHistoryDoc.data)
    ...
    // Safely update admission history
    const data = await PatientAdmissionHistory.findByIdAndUpdate(
      req.params.id,
      {
        ...modifiedBody,
        resourceAllocation: req.body?.resourceAllocation ?? admissionHistoryDoc.data?.resourceAllocation ?? [],
      },
      { new: true }
    );
    if (!data) return next(new AppError('Please Send Valid Patient Admission History ID', 400));

    // Create revision safely
...
console.log("Uploaded files:", req.files);
    ...
    console.log("Updated DB admissionChecklist:", updatedData?.admissionChecklist);
